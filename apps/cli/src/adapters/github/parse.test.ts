...
describe("parseStarredPage", () => {
  ...
  test("produces one NormalizedItem per starred repo", () => {
    ...
    expect(items.map((i) => i.title)).toEqual(["firecrawl/firecrawl", "mastra-ai/mastra"]);
  });
  ...
  test("starred_at becomes an exact saved_at — the first in the library", () => {
    ...
    expect(firecrawl!.savedAt).toBe(Math.floor(Date.parse("2026-07-12T09:14:03Z") / 1000));
  ...
  test("node_id is the upsert key, not the numeric id", () => {
    expect(items[0]!.externalId).toBe("R_kgDOMHgQ7g");
  });
  ...
  test("tolerates a bare repo array without the star+json wrapper", () => {
    const bare = parseStarredPage(raw.starred.map((e: { repo: unknown }) => e.repo), ctx);
  ...
  test("skips entries with no id at all", () => {
    expect(parseStarredPage({ starred: [{ repo: { full_name: "x/y" } }] }, ctx)).toHaveLength(0);
  });
});
