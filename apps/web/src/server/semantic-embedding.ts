...
export async function prepareSemanticItem(
	...
	if (settings.semanticSearchEnabled !== 1 || settings.semanticIndexPaused === 1) {
		return { parentHash: "", generation: settings.semanticGeneration, chunks: [], vectors: [] };
	}
	...
	if (current.semanticSearchEnabled !== 1 || current.semanticIndexPaused === 1 || current.semanticGeneration !== settings.semanticGeneration || current.embeddingModel !== provider.model) {
		await discardSemanticChunks(db, staged.chunks.map((chunk) => chunk.id));
		return { parentHash: staged.parentHash, generation: settings.semanticGeneration, chunks: [], vectors: [] };
	}
	const pending = staged.chunks.filter((chunk) =>
		chunk.status !== "complete" ||
		(options.localCache && !options.localCache.hasRecord(chunk.id, chunk.contentHash)),
	);
	const vectors: PreparedSemanticItem["vectors"] = [];
	for (let offset = 0; offset < pending.length; offset += 16) {
		...
		if (embeddings.length !== batch.length) throw new AiProviderError("malformed", "embedding provider returned an incomplete chunk batch");
		for (let index = 0; index < batch.length; index += 1) {
			...
			vectors.push({ id: chunk.id, contentHash: chunk.contentHash, values });
		}
	}
	return { parentHash: staged.parentHash, generation: settings.semanticGeneration, chunks: staged.chunks, vectors };
}
