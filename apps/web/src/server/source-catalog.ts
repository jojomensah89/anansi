...
export const SOURCE_CATALOG: readonly SourceDefinition[] = [
  ...
  },
  {
    ...
    name: "GitHub stars",
    host: "github.com",
    note: "Extension-based starred repository capture is coming next.",
    support: "coming_next",
    mode: "page",
    toggleable: false,
    requiresExtension: true,
...
export function sourceCatalogueResponse(
  ...
  return {
    ...
    sources: SOURCE_CATALOG.map((definition) => ({
      ...
      runtime: extension.sources[definition.source as keyof typeof extension.sources] ?? null,
    })),
  };
}

export function isToggleableSource(source: string): boolean {
  return SOURCE_CATALOG.some((entry) => entry.source === source && entry.toggleable);
}
