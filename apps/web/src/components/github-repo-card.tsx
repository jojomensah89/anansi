import { compact, type ItemRow, shortDate } from "../lib/api.ts";
...
export function githubLanguageColor(language?: string | null): string {
	return (
		LANGUAGE_COLORS[language?.trim().toLowerCase() ?? ""] ??
		NEUTRAL_LANGUAGE_COLOR
	);
}
...
function Stat({ label, value }: { label: string; value: number | undefined }) {
	...
	return (
		<span
			style={{ display: "flex", flexDirection: "column", gap: 2, minWidth: 44 }}
		>
			<span style={{ color: "#4b5563", fontSize: 11, lineHeight: 1.1 }}>
				{compact(value)}
			</span>
			<span style={{ color: "#7b8490", fontSize: 8.5, lineHeight: 1.1 }}>
				{label}
			</span>
		</span>
	);
...
export function GithubRepoCard({ item, footer }: { item: ItemRow; footer?: ReactNode }) {
		...
		<>
			...
			>
				<div style={{ display: "flex", alignItems: "center", gap: 8 }}>
					<Avatar src={item.authorAvatar} seed={owner} square size={42} />
					<span
			...
			>
				...
				<div
					title={language ?? "Language unavailable"}
					style={{
						height: 5,
						background: language ? languageColor : "#e5e7eb",
					}}
				/>
			</div>
			...
			>
				...
				>
					...
					<SourceMark source="github" size={20} />
				</span>
			</div>
		</>
	);
