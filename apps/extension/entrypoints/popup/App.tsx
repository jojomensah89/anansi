...
export default function App() {
  ...
  const describe = (s: SourceConfig) => {
    ...
    if (st?.message && st.message !== "running…") return { text: st.message, tone: st.failed ? S.warn : S.muted };
    if (st?.message === "running…") return { text: "importing…", tone: S.accent };
    if (held > 0) return { text: `Up to date · ${held.toLocaleString()}`, tone: S.faint };
    if (s.mode === "observe") return { text: "Scroll your favourites to capture", tone: S.faint };
            ...
            <div key={s.source} style={{ display: "flex", alignItems: "center", gap: 11, padding: "11px 16px", borderTop: `1px solid ${S.line}` }}>
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 11.5,
                  flexShrink: 0,
                  background: st?.message === "running…" ? S.raised : S.card,
                  color: S.text,
                }}
              >
                {observe ? "Watch" : st?.message === "running…" ? "…" : "Import"}
              </button>
            </div>
          );
        })}
...
function Mark({ source }: { source: string }) {
  const p = { width: 14, height: 14, viewBox: "0 0 24 24", fill: "currentColor" } as const;
  if (source === "reddit")
    return (
      <svg {...p}>
